...
# added for create datatime as exp_id
...
def get_dump_path(params):
    ...
    sweep_path = os.path.join(sweep_path,params.oracle.upper() + 'oracle')
    if not os.path.exists(sweep_path):
        os.makedirs(sweep_path)
        # subprocess.Popen("mkdir -p %s" % sweep_path, shell=True).wait()
    ...
    # otherwise, it is generated with time
    if params.exp_id == '':
...
def shuf_order(langs, params=None, n=5):
    ...
    langs: params.mlm_steps: [(ar, None), (bg, None), ..., (en, ar), (en, bg), ...]
    """
    if len(langs) == 0:
        return []
