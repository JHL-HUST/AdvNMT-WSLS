...
def set_pretrain_emb(model, dico, word2id, embeddings):
    ...
    n_found = 0
    with torch.no_grad():
        # dico重写了len, get方法，所以可以直接按照下面这种方式使用
        for i in range(len(dico)):
            idx = word2id.get(dico[i], None)
            if idx is None:
                continue
            n_found += 1
            # TransformerModel的输入: 词向量的编码
            model.embeddings.weight[i] = embeddings[idx].cuda()
            # 为什么要把权重初始化为这个？？？
            # 猜测：输入是位置向量？和字典[dim, N]相乘，正好可以得到每个词所对应的概率
...
def build_model(params, dico):
    ...
    if params.encoder_only:
        ...
        # 加载已经训练好的模型
        if params.reload_model != '':
            # 参数: --reload_model 'mlm_enfr_1024.pth,mlm_enfr_1024.pth'
